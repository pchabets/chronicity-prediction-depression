...
# ## remove analytes that have one unique value, or predictors having both of the following characteristics:
# #   1) very few unique values relative to the number of samples (<5% of samples)
# #   2) the ratio of the frequency of the most common value to the frequency of the 2nd most common value is large (>95/5)
# whole_set <- whole_set[,-nearZeroVar(whole_set, uniqueCut = 5)] # keep 201 of 237 analytes with non-nearZeroVariance (removes 36 analytes)
...
#preprocess trainingsdata
prepTrain <- preProcess(segTrain[, 3:11],
                   #used preProcessing arguments only for bigFive and IDS numerical data (ordinal categorical severity data excluded)
                   method = c("medianImpute"))
transformedTrain <- predict(prepTrain, newdata = segTrain[, 3:11])
segTrain[3:11] <- transformedTrain

# similarly preprocess testdata
transformedTest <- predict(prepTrain, newdata = segTest[, 3:11])
segTest[3:11] <- transformedTest
...
# tune_grid <- expand.grid(nrounds = 666,
#                          eta = 0.5867545,
#                          max_depth = c(2, 3, 4, 5, 6),
#                          gamma = 0.5867545,
#                          colsample_bytree = 0.6740495,
#                          min_child_weight = 4,
#                          subsample = 0.779125)
tic()
RFTune <- train(x = segTrain[,-1],
                 y = segTrain[,1],
                 method = "rf",
...
RFTune$finalModel$tuneValue
plot(RFTune)

# saveRDS(RFTune, "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/RFTune3.RDS")
RFTune <- readRDS("/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/RFTune3.RDS")

## predictions on test set:
RFPred <- predict(RFTune, segTest[,-1])
cm <- confusionMatrix(RFPred, segTest[,1])
draw_confusion_matrix(cm, ratio = TRUE)

#Get predicted class probabilities so we can build ROC curve.
RFProbs <- predict(RFTune, segTest[,-1], type="prob")
head(RFProbs)

#build ROC curve
RFROC <- roc(segTest[,1], RFProbs[,"remitted"])
# saveRDS(RFROC, "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/rf_roc_2ychr_clin.RDS")
auc(RFROC)

#plot ROC curve
ggplot(data.frame(specificity = RFROC$specificities, sensitivity = RFROC$sensitivities), aes(specificity, sensitivity)) +
  geom_path() +
  scale_x_reverse() +
  geom_abline(intercept = 1, slope = 1, colour='grey') +
  coord_fixed(ratio = 1, xlim = NULL, ylim = NULL, expand = TRUE, clip = "on") +
  theme_classic() +
  labs(title = paste0("AUROC =", signif(auc(RFROC), 3)))


# Variable importance
var_imp <- varImp(RFTune, scale = FALSE)
var_imp$importance
gain <- var_imp$importance %>% 
  mutate(feature = rownames(var_imp$importance)) %>% 
  relocate(feature, .before = Overall) %>% 
