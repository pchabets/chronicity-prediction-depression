...
# saveRDS(rfTune, "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/2-yearChronicity/output/RFTune_batchCorrected.RDS")
RFTune <- readRDS("/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/2-yearChronicity/output/RFTune_batchCorrected.RDS")
...
accuracies <- list()
for(i in seq(0.4, 0.6, 0.005)){
  pred_ <- factor(ifelse(predict(RFTune, segTest[,-1], type="prob")$remitted>i,"remitted","not_remitted"), levels = c("remitted", "not_remitted"))
  cm_ <- confusionMatrix(pred_, segTest[,1])
  accuracies <- append(accuracies, cm_$overall[1])
}
names(accuracies) <- seq(0.4, 0.6, 0.005)
opt_thresh <- as.numeric(names(accuracies[accuracies == max(sapply(accuracies, max))]))[1]
rfPred <- factor(ifelse(predict(RFTune, segTest[,-1], type="prob")$remitted>opt_thresh,"remitted","not_remitted"), levels = c("remitted", "not_remitted"))
cm <- confusionMatrix(rfPred, as.factor(segTest[,1]))
draw_confusion_matrix(cm, ratio = TRUE)
...
# write_excel_csv(analyte_importance, "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/RF_analyte_importance.csv")
...
# saveRDS(RFROC, file = "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/roc_2ychronicity_proteomics_RF.RDS")
auc(RFROC)
