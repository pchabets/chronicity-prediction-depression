...
#                            number = 10,
#                            repeats = 10,
#                            summaryFunction = twoClassSummary, #should be set to default if metric = accuracy
#                            search = "random",
#                            classProbs = TRUE,
#                            seeds = seeds,
#                            allowParallel = TRUE,
...
# tune_grid <- expand.grid(alpha = seq(from = 0.05, to = 0.15, length.out = 50),
#                          #minimum alpha based on best fit from random parameter search (tuneLength=100)
#                          lambda = seq(0.001, 0.004, length.out =1000)
#                          #minimum lambda based on best fit from random parameter search (tuneLength=100)
#                          )


# tune_grid <- expand.grid(alpha = seq(0,1,0.2),
...
elTune <- train(x = segTrain[,-1],
                ...
                # preProcess = c("center", "scale"), # needed for ridge/lasso regression
                verbose = TRUE)
toc_hour()

# saveRDS(elTune, "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/elTune3.RDS")
elTune <- readRDS("/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/elTune3.RDS")

# See final alpha and lambda parameters
elTune$finalModel$tuneValue
...
coef_abs <- data.frame(variable = rownames(coef), coefficient = coef[,1])
coef_abs <- coef_abs %>%
  dplyr::slice(-1) %>% 
  arrange(desc(abs(coefficient)))
  
coef_abs <- rbind(data.frame(variable = "intercept", coefficient = coef[1,]), coef_abs) %>% 
  mutate(coefficient = round(coefficient, 3))

# write coefficients to file
# write_csv(coef_abs, "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/2-yearChronicity/clinical_variables/output/coefficients_elnet_ranked.csv")
...
# saveRDS(elROC, "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/eln_roc_2ychr_clin.RDS")
auc(elROC)
