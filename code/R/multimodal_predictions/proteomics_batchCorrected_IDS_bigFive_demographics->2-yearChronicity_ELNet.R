...
elTune <- train(x = segTrain[,-1],
                 ...
                 preProcess = c("medianImpute", "center", "scale"), # will only be performed on clinical variables as proteomic variables are already imputed, center + scale for elnet
                 verbose = TRUE)

toc_hour()
elTune$finalModel

# saveRDS(elTune, "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/elTune2.RDS")
elTune <- readRDS("/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/elTune2.RDS")
...
for(i in seq(0.4, 0.6, 0.01)){
  pred_ <- factor(ifelse(predict(elTune, segTest[,-1], type="prob")$remitted>i,"remitted","not_remitted"), levels = c("remitted", "not_remitted"))
...
elPred <- factor(ifelse(predict(elTune, segTest[,-1], type="prob")$remitted>opt_thresh,"remitted","not_remitted"), levels = c("remitted", "not_remitted"))

# Confusion matrix
cm <- confusionMatrix(elPred, segTest[,1])
draw_confusion_matrix(cm, ratio = TRUE)
var_imp <- varImp(elTune, scale = FALSE)
var_imp$importance
...
# write_excel_csv(analyte_importance, "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/prot_clin_analyte_importance_elNET.csv")
...
# saveRDS(elROC, file = "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/roc_2ychronicity_prot_clin_ELNET.RDS")
auc(elROC)
ci.auc(elROC)

#plot ROC curve
ggplot(data.frame(specificity = elROC$specificities, sensitivity = elROC$sensitivities), aes(specificity, sensitivity)) +
  geom_path() +
  scale_x_reverse() +
