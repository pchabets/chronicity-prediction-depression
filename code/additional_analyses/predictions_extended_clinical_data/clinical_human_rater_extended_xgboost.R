...
# Load data
set <- read_csv("data/clinical_human_rater_extended_data.csv") %>% 
...
## partition data
set.seed(42)
trainIndex <- createDataPartition(y=set$Remitted_depression, times=1, p=0.8, list=F)

# training set
segTrain <- as.data.frame(set[trainIndex,-1])

# testing set 
segTest <- as.data.frame(set[-trainIndex,-1])

table(segTrain$Remitted_depression) #non-remitted/remitted = 312/322
table(segTest$Remitted_depression) #non-remitted/remitted  = 78/80
...
##train model
tic() 
xgbTune <- train(x = segTrain %>% select(-Remitted_depression),
                 ...
                 verbose = TRUE)
toc_hour()
# saveRDS(xgbTune, "data/XGbTune_clinical_extended_data.RDS")

xgbTune <- readRDS("data/XGbTune_clinical_extended_data.RDS")
...
auc(XGbROC)
# save to file
# saveRDS(XGbROC, "/Users/philippehabets/Dropbox/STRESS_INDEX/scripts/Predictions_explorative/R.scripts/data/xgb_clinical_extended.RDS")
...
# Plot performance with other raters' performance
library(reshape2)
raters <- read_csv("data/raters_vs_xgboost_initial.csv") 
raters_new_data <- data.frame('rater'= raters$rater, 
                      "clinical.data.only" = raters$clinical.data.only, 
                      "extended.clinical" = c(signif(cm$byClass['Balanced Accuracy'],2), raters$with.added.data[-1]),
                      "added.proteomics" = c(raters$with.added.data[1], rep(NA, 5)))


# plotting of results
plot_accs <- melt(raters_new_data, variable.name = "used data", value.name = "balanced accuracy") %>% 
  mutate(`used data` = recode(`used data`, clinical.data.only = "10 clinical features", extended.clinical = "with additional clinical info", added.proteomics = "with proteomics added"))
plot <- ggplot(plot_accs, aes(x = `balanced accuracy`, y = rater, fill = `used data`)) +
  ...
  geom_vline(xintercept=c(.63, 0.65, .71), linetype="dotted") +
  annotate("text", x = c(0.61,0.67,0.73), y=6.3, label = c("0.63","0.65","0.71"), size=6) +
  annotate("text", x = c(0.51,0.55), y=5.3, label = c("0.51","0.55"), size=6) +
  theme_gray(base_size = 24) +
  theme(legend.justification = 'top')
plot

# ggsave("/Users/philippehabets/Dropbox/STRESS_INDEX/manuscripts/prediction_2ychronicity/BiolPsy/Figures/output/human_vs_machine_extended.pdf",
#        plot,
#        width = 35, height = 15, units = "cm")

# average performance of raters excluding rater that was part of study design (although single blinded on models' performance)
raters_excl <- raters[4:6,] 
mean(raters_excl$clinical.data.only)
mean(raters_excl$with.added.data)
